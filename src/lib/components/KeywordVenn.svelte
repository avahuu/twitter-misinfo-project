<script lang="ts">
  import { fade } from 'svelte/transition';
  import { onMount } from 'svelte';

  // ── Types ─────────────────────────────────────────────────────────────────────
  interface Kw { cn: string; en: string; pct: number; }
  interface FBubble extends Kw { r: number; x: number; y: number; vx: number; vy: number; }

  // ── View state ────────────────────────────────────────────────────────────────
  let view: 'venn' | 'bubble' = 'venn';
  let selectedId: 'usa' | 'syd' | 'cao' | null = null;
  let hoverRegion: string | null = null;
  let clientX = 0, clientY = 0;
  let svgEl: SVGSVGElement;

  // ── Scatter dots (generated in onMount for SSR safety) ────────────────────────
  let usaDots: {x:number,y:number}[] = [];
  let caoDots: {x:number,y:number}[] = [];
  let sydDots: {x:number,y:number}[] = [];

  // ── Bubble layout result ──────────────────────────────────────────────────────
  let bubbles: FBubble[] = [];

  // ── Venn geometry (circle sizes proportional to sqrt(posts)) ─────────────────
  //   usa: 8013 posts → r=125   syd: 7474 → r=115   cao: 3517 → r=95
  const CIRC = {
    usa: { cx:330, cy:220, r:125, color:'#4da6ff', dim:'rgba(77,166,255,0.07)',  handle:'@usa912152217', posts:8013 },
    cao: { cx:485, cy:220, r:95,  color:'#27d191', dim:'rgba(39,209,145,0.07)',  handle:'@CaoChangqing',  posts:3517 },
    syd: { cx:410, cy:360, r:115, color:'#f5a623', dim:'rgba(245,166,35,0.07)', handle:'@SydneyDaddy1',  posts:7474 },
  } as const;

  // ── Keyword data (from keywords_comp.csv) ─────────────────────────────────────
  const USA_KWS: Kw[] = [
    {cn:'川普',en:'Trump',pct:46.8},{cn:'美国',en:'United States',pct:44.4},
    {cn:'总统',en:'President',pct:30.9},{cn:'政府',en:'Government',pct:27.1},
    {cn:'这是',en:'This is',pct:17.6},{cn:'拜登',en:'Biden',pct:17.3},
    {cn:'可能',en:'Possible',pct:14.6},{cn:'国家',en:'Country',pct:14.0},
    {cn:'现在',en:'Now',pct:13.6},{cn:'今天',en:'Today',pct:12.7},
    {cn:'知道',en:'Know',pct:12.2},{cn:'民主党',en:'Democrats',pct:12.1},
    {cn:'联邦',en:'Federation',pct:11.4},{cn:'支持',en:'Support',pct:11.3},
    {cn:'非法',en:'Illegal',pct:10.8},{cn:'宣布',en:'Announce',pct:10.7},
    {cn:'媒体',en:'Media',pct:10.3},{cn:'移民',en:'Immigration',pct:10.0},
    {cn:'问题',en:'Question',pct:9.9},{cn:'民猪',en:'Folk Pig',pct:9.7},
    {cn:'不会',en:"Won't",pct:9.4},{cn:'这种',en:'This Kind',pct:9.3},
    {cn:'法律',en:'Law',pct:9.1},{cn:'亿美元',en:'$Billions',pct:8.6},
    {cn:'选举',en:'Election',pct:8.7},{cn:'大选',en:'Gen. Election',pct:8.2},
    {cn:'马斯克',en:'Musk',pct:7.2},{cn:'共和党',en:'Republicans',pct:6.5},
    {cn:'战争',en:'War',pct:6.5},{cn:'乌克兰',en:'Ukraine',pct:5.1},
  ];

  const CAO_KWS: Kw[] = [
    {cn:'美国',en:'United States',pct:65.8},{cn:'川普',en:'Trump',pct:61.1},
    {cn:'总统',en:'President',pct:53.0},{cn:'左派',en:'Leftist',pct:35.8},
    {cn:'支持',en:'Support',pct:27.4},{cn:'拜登',en:'Biden',pct:26.8},
    {cn:'这是',en:'This is',pct:24.2},{cn:'今天',en:'Today',pct:23.2},
    {cn:'政府',en:'Government',pct:23.0},{cn:'国家',en:'Country',pct:22.1},
    {cn:'政治',en:'Politics',pct:19.9},{cn:'可能',en:'Possible',pct:19.8},
    {cn:'民主党',en:'Democrats',pct:18.9},{cn:'左媒',en:'Left Media',pct:17.0},
    {cn:'这种',en:'This Kind',pct:16.3},{cn:'白宫',en:'White House',pct:15.7},
    {cn:'现在',en:'Now',pct:15.1},{cn:'世界',en:'World',pct:14.8},
    {cn:'问题',en:'Question',pct:14.7},{cn:'哈里斯',en:'Harris',pct:14.0},
    {cn:'人民',en:'People',pct:14.0},{cn:'全球',en:'Global',pct:13.8},
    {cn:'保守派',en:'Conservative',pct:12.8},{cn:'中国',en:'China',pct:12.5},
    {cn:'媒体',en:'Media',pct:12.2},{cn:'乌克兰',en:'Ukraine',pct:11.9},
    {cn:'共和党',en:'Republicans',pct:10.4},{cn:'亿美元',en:'$Billions',pct:10.4},
    {cn:'战争',en:'War',pct:10.2},{cn:'移民',en:'Immigration',pct:10.1},
    {cn:'非法',en:'Illegal',pct:9.0},{cn:'马斯克',en:'Musk',pct:9.1},
    {cn:'黑人',en:'Black People',pct:8.2},{cn:'欧洲',en:'Europe',pct:8.4},
    {cn:'哈马斯',en:'Hamas',pct:5.5},{cn:'普京',en:'Putin',pct:5.7},
    {cn:'台湾',en:'Taiwan',pct:5.4},{cn:'日本',en:'Japan',pct:5.4},
    {cn:'伊朗',en:'Iran',pct:5.0},{cn:'林斯基',en:'Linsky',pct:6.7},
  ];

  const SYD_KWS: Kw[] = [
    {cn:'川普',en:'Trump',pct:12.7},{cn:'美国',en:'United States',pct:12.2},
    {cn:'中国',en:'China',pct:11.3},{cn:'中共',en:'CCP',pct:5.4},
    {cn:'支持',en:'Support',pct:4.7},{cn:'可能',en:'Possible',pct:4.6},
    {cn:'这是',en:'This is',pct:4.6},{cn:'政府',en:'Government',pct:4.2},
    {cn:'国家',en:'Country',pct:3.8},{cn:'伊朗',en:'Iran',pct:3.3},
    {cn:'这种',en:'This Kind',pct:3.3},{cn:'澳洲',en:'Australia',pct:3.2},
    {cn:'以色列',en:'Israel',pct:3.2},{cn:'日本',en:'Japan',pct:3.2},
    {cn:'台湾',en:'Taiwan',pct:3.1},{cn:'知道',en:'Know',pct:3.1},
    {cn:'希望',en:'Hope',pct:3.0},{cn:'现在',en:'Now',pct:3.7},
    {cn:'问题',en:'Question',pct:2.8},{cn:'今天',en:'Today',pct:2.2},
    {cn:'马斯克',en:'Musk',pct:2.8},{cn:'媒体',en:'Media',pct:2.6},
    {cn:'政治',en:'Politics',pct:2.8},{cn:'乌克兰',en:'Ukraine',pct:2.9},
    {cn:'民主党',en:'Democrats',pct:2.1},{cn:'总统',en:'President',pct:2.1},
    {cn:'移民',en:'Immigration',pct:1.7},{cn:'哈里斯',en:'Harris',pct:2.1},
    {cn:'宣布',en:'Announce',pct:2.3},{cn:'不会',en:"Won't",pct:2.2},
    {cn:'俄国',en:'Russia',pct:2.1},{cn:'习近平',en:'Xi Jinping',pct:2.0},
    {cn:'关税',en:'Tariff',pct:2.4},{cn:'民调',en:'Poll',pct:1.6},
    {cn:'孩子',en:'Children',pct:1.6},{cn:'澳洲',en:'Australia',pct:3.2},
  ];

  // ── Intersection keyword sets (pre-computed from CSV analysis) ─────────────────
  // All three accounts share these keywords
  const INT_ALL: Kw[] = [
    {cn:'川普',en:'Trump',pct:12.7},{cn:'美国',en:'United States',pct:12.2},
    {cn:'总统',en:'President',pct:2.1},{cn:'政府',en:'Government',pct:4.2},
    {cn:'这是',en:'This is',pct:4.6},{cn:'可能',en:'Possible',pct:4.6},
    {cn:'国家',en:'Country',pct:3.8},{cn:'现在',en:'Now',pct:3.7},
    {cn:'今天',en:'Today',pct:2.2},{cn:'民主党',en:'Democrats',pct:2.1},
    {cn:'支持',en:'Support',pct:4.7},{cn:'媒体',en:'Media',pct:2.6},
    {cn:'移民',en:'Immigration',pct:1.7},{cn:'问题',en:'Question',pct:2.8},
    {cn:'这种',en:'This Kind',pct:3.3},{cn:'马斯克',en:'Musk',pct:2.8},
    {cn:'乌克兰',en:'Ukraine',pct:2.9},
  ];
  // USA ∩ CAO only
  const INT_USA_CAO: Kw[] = [
    {cn:'拜登',en:'Biden',pct:17.3},{cn:'非法',en:'Illegal',pct:10.8},
    {cn:'亿美元',en:'$Billions',pct:8.6},{cn:'共和党',en:'Republicans',pct:6.5},
    {cn:'战争',en:'War',pct:6.5},
  ];
  // USA ∩ SYD only
  const INT_USA_SYD: Kw[] = [
    {cn:'知道',en:'Know',pct:12.2},{cn:'宣布',en:'Announce',pct:10.7},
    {cn:'不会',en:"Won't",pct:9.4},
  ];
  // CAO ∩ SYD only
  const INT_CAO_SYD: Kw[] = [
    {cn:'中国',en:'China',pct:11.3},{cn:'政治',en:'Politics',pct:2.8},
    {cn:'哈里斯',en:'Harris',pct:2.1},{cn:'以色列',en:'Israel',pct:3.2},
    {cn:'日本',en:'Japan',pct:3.2},{cn:'台湾',en:'Taiwan',pct:3.1},
    {cn:'伊朗',en:'Iran',pct:3.3},{cn:'俄国',en:'Russia',pct:2.1},
  ];

  const intCns = new Set([...INT_ALL,...INT_USA_CAO,...INT_USA_SYD,...INT_CAO_SYD].map(k=>k.cn));

  const USA_ONLY = USA_KWS.filter(k=>!intCns.has(k.cn)).slice(0,6);
  const CAO_ONLY = CAO_KWS.filter(k=>!intCns.has(k.cn)).slice(0,6);
  const SYD_ONLY = SYD_KWS.filter(k=>!intCns.has(k.cn)).slice(0,6);

  const REGION_KWS: Record<string,Kw[]> = {
    usa: USA_ONLY, cao: CAO_ONLY, syd: SYD_ONLY,
    usa_cao: INT_USA_CAO, usa_syd: INT_USA_SYD,
    cao_syd: INT_CAO_SYD, all: INT_ALL,
  };

  const REGION_COUNTS: Record<string,number> = {
    usa: USA_ONLY.length, cao: CAO_ONLY.length, syd: SYD_ONLY.length,
    usa_cao: INT_USA_CAO.length, usa_syd: INT_USA_SYD.length,
    cao_syd: INT_CAO_SYD.length, all: INT_ALL.length,
  };

  // Approximate label positions for each region (verified in-circle)
  const REGION_LABELS: Record<string,{x:number,y:number}> = {
    usa:     { x:220, y:200 },
    cao:     { x:555, y:200 },
    syd:     { x:408, y:450 },
    usa_cao: { x:407, y:210 },
    usa_syd: { x:342, y:300 },
    cao_syd: { x:478, y:300 },
    all:     { x:407, y:264 },
  };

  const REGION_TITLES: Record<string,string> = {
    usa:'Unique to usa912152217', cao:'Unique to CaoChangqing', syd:'Unique to SydneyDaddy1',
    usa_cao:'usa ∩ CaoChangqing only', usa_syd:'usa ∩ SydneyDaddy1 only',
    cao_syd:'CaoChangqing ∩ SydneyDaddy1 only', all:'Shared by all three',
  };

  // ── Geometry helpers ──────────────────────────────────────────────────────────
  function d2(x1:number,y1:number,x2:number,y2:number) {
    return Math.sqrt((x2-x1)**2+(y2-y1)**2);
  }

  function getRegion(mx:number,my:number): string|null {
    const A = d2(mx,my,CIRC.usa.cx,CIRC.usa.cy) < CIRC.usa.r;
    const B = d2(mx,my,CIRC.cao.cx,CIRC.cao.cy) < CIRC.cao.r;
    const C = d2(mx,my,CIRC.syd.cx,CIRC.syd.cy) < CIRC.syd.r;
    if (A&&B&&C) return 'all';
    if (A&&B) return 'usa_cao';
    if (A&&C) return 'usa_syd';
    if (B&&C) return 'cao_syd';
    if (A) return 'usa';
    if (B) return 'cao';
    if (C) return 'syd';
    return null;
  }

  function onSVGMouseMove(e: MouseEvent) {
    clientX = e.clientX; clientY = e.clientY;
    const rect = svgEl.getBoundingClientRect();
    const mx = (e.clientX - rect.left) * (780 / rect.width);
    const my = (e.clientY - rect.top)  * (520 / rect.height);
    hoverRegion = getRegion(mx, my);
  }

  function onSVGMouseLeave() { hoverRegion = null; }

  function onCircleClick(id: 'usa'|'cao'|'syd') {
    selectedId = id;
    const kwMap: Record<string,Kw[]> = { usa:USA_KWS, cao:CAO_KWS, syd:SYD_KWS };
    bubbles = forceLayout(kwMap[id]);
    view = 'bubble';
  }

  // ── Scatter dots inside a circle ──────────────────────────────────────────────
  function circDots(cx:number,cy:number,r:number,n:number) {
    const arr:{x:number,y:number}[] = [];
    while(arr.length<n) {
      const a=Math.random()*2*Math.PI, d=Math.sqrt(Math.random())*r*0.9;
      arr.push({x:cx+Math.cos(a)*d, y:cy+Math.sin(a)*d});
    }
    return arr;
  }

  // ── Force layout for bubble chart ─────────────────────────────────────────────
  function forceLayout(kws: Kw[]): FBubble[] {
    const BW=720, BH=460;
    const maxP = Math.max(...kws.map(k=>k.pct));
    const items: FBubble[] = kws.slice(0,30).map(k=>({
      ...k,
      r: Math.round(14 + (k.pct/maxP)*50),
      x: BW/2 + (Math.random()-0.5)*100,
      y: BH/2 + (Math.random()-0.5)*100,
      vx:0, vy:0,
    }));

    for(let iter=0;iter<500;iter++){
      for(let i=0;i<items.length;i++){
        for(let j=i+1;j<items.length;j++){
          const dx=items[j].x-items[i].x, dy=items[j].y-items[i].y;
          const dist=Math.sqrt(dx*dx+dy*dy)||0.01;
          const minD=items[i].r+items[j].r+6;
          if(dist<minD){
            const f=((minD-dist)/dist)*0.5;
            items[i].vx-=dx*f; items[i].vy-=dy*f;
            items[j].vx+=dx*f; items[j].vy+=dy*f;
          }
        }
        items[i].vx+=(BW/2-items[i].x)*0.012;
        items[i].vy+=(BH/2-items[i].y)*0.012;
        const pad=items[i].r+6;
        if(items[i].x<pad)    items[i].vx+=(pad-items[i].x)*0.3;
        if(items[i].x>BW-pad) items[i].vx+=(BW-pad-items[i].x)*0.3;
        if(items[i].y<pad)    items[i].vy+=(pad-items[i].y)*0.3;
        if(items[i].y>BH-pad) items[i].vy+=(BH-pad-items[i].y)*0.3;
      }
      for(let i=0;i<items.length;i++){
        items[i].vx*=0.82; items[i].vy*=0.82;
        items[i].x+=items[i].vx; items[i].y+=items[i].vy;
      }
    }
    return items;
  }

  onMount(()=>{
    usaDots = circDots(CIRC.usa.cx, CIRC.usa.cy, CIRC.usa.r, 180);
    caoDots = circDots(CIRC.cao.cx, CIRC.cao.cy, CIRC.cao.r, 130);
    sydDots = circDots(CIRC.syd.cx, CIRC.syd.cy, CIRC.syd.r, 155);
  });

  // ── Reactive derivations ──────────────────────────────────────────────────────
  $: tooltipKws  = hoverRegion ? (REGION_KWS[hoverRegion]||[]).slice(0,8) : [];
  $: tooltipTitle = hoverRegion ? (REGION_TITLES[hoverRegion]||'') : '';
  $: selCirc = selectedId ? CIRC[selectedId] : null;

  // For region counts in the Venn
  const regions = Object.keys(REGION_LABELS) as (keyof typeof REGION_LABELS)[];

  function circleOpacity(id: string): number {
    if (!hoverRegion) return 0.82;
    return hoverRegion.includes(id) ? 1 : 0.18;
  }
</script>

<section class="kv">

  <!-- ── FRAME 1: Venn Diagram ─────────────────────────────────────────────────── -->
  {#if view === 'venn'}
  <div transition:fade={{duration:350}}>
    <div class="kv-head">
      <h2 class="kv-title">Keyword Overlap Across Accounts</h2>
      <p class="kv-sub">Hover intersections to see shared keywords · click a circle to explore</p>
    </div>

    <!-- Account legend badges -->
    <div class="legend-row">
      {#each Object.values(CIRC) as c}
        <button class="acct-pill" style="color:{c.color}; border-color:{c.color}40; background:{c.dim}"
          on:click={()=>onCircleClick(c.handle.includes('usa')?'usa':c.handle.includes('Cao')?'cao':'syd')}>
          {c.handle} · {c.posts.toLocaleString()} posts
        </button>
      {/each}
    </div>

    <div class="venn-wrap">
      <svg bind:this={svgEl} viewBox="0 0 780 520" class="venn-svg"
        on:mousemove={onSVGMouseMove} on:mouseleave={onSVGMouseLeave}
        role="img" aria-label="Venn diagram of keyword overlaps">

        <defs>
          <!-- Organic hand-drawn distortion filter -->
          <filter id="hd" x="-8%" y="-8%" width="116%" height="116%">
            <feTurbulence type="fractalNoise" baseFrequency="0.04" numOctaves="3" result="n"/>
            <feDisplacementMap in="SourceGraphic" in2="n" scale="4"
              xChannelSelector="R" yChannelSelector="G"/>
          </filter>
        </defs>

        <!-- Scatter dots per circle (background texture) -->
        {#each usaDots as d}
          <circle cx={d.x} cy={d.y} r="1.3" fill={CIRC.usa.color} opacity="0.14"/>
        {/each}
        {#each caoDots as d}
          <circle cx={d.x} cy={d.y} r="1.3" fill={CIRC.cao.color} opacity="0.14"/>
        {/each}
        {#each sydDots as d}
          <circle cx={d.x} cy={d.y} r="1.3" fill={CIRC.syd.color} opacity="0.14"/>
        {/each}

        <!-- Soft fill per circle -->
        {#each Object.entries(CIRC) as [id, c]}
          <circle cx={c.cx} cy={c.cy} r={c.r}
            fill={c.dim} stroke="none"
            opacity={circleOpacity(id)}
            style="transition:opacity 0.2s"/>
        {/each}

        <!-- Dashed organic circle outlines -->
        {#each Object.entries(CIRC) as [id, c]}
          <circle cx={c.cx} cy={c.cy} r={c.r}
            fill="none"
            stroke={c.color}
            stroke-width="1.8"
            stroke-dasharray="7 3.5"
            opacity={circleOpacity(id)}
            filter="url(#hd)"
            style="transition:opacity 0.2s; cursor:pointer"
            on:click={()=>onCircleClick(id as any)}
            role="button"
            aria-label="Explore {c.handle}"
          />
        {/each}

        <!-- Region count labels -->
        {#each regions as rk}
          <text
            x={REGION_LABELS[rk].x} y={REGION_LABELS[rk].y}
            text-anchor="middle" dominant-baseline="middle"
            font-family="Montserrat,sans-serif"
            font-size={rk==='all' ? 20 : 14}
            font-weight={rk==='all' ? 800 : 600}
            fill="rgba(255,255,255,{rk==='all' ? 0.75 : 0.5})"
            opacity={!hoverRegion || hoverRegion===rk ? 1 : 0.25}
            style="transition:opacity 0.2s; pointer-events:none">
            {REGION_COUNTS[rk]}
          </text>
        {/each}

        <!-- Account handle labels outside circles -->
        <text x={CIRC.usa.cx-30} y={CIRC.usa.cy - CIRC.usa.r - 14}
          font-family="Montserrat,sans-serif" font-size="12" font-weight="700"
          fill={CIRC.usa.color} text-anchor="middle" opacity={circleOpacity('usa')}
          style="pointer-events:none;transition:opacity 0.2s">@usa912152217</text>

        <text x={CIRC.cao.cx+10} y={CIRC.cao.cy - CIRC.cao.r - 14}
          font-family="Montserrat,sans-serif" font-size="12" font-weight="700"
          fill={CIRC.cao.color} text-anchor="middle" opacity={circleOpacity('cao')}
          style="pointer-events:none;transition:opacity 0.2s">@CaoChangqing</text>

        <text x={CIRC.syd.cx} y={CIRC.syd.cy + CIRC.syd.r + 20}
          font-family="Montserrat,sans-serif" font-size="12" font-weight="700"
          fill={CIRC.syd.color} text-anchor="middle" opacity={circleOpacity('syd')}
          style="pointer-events:none;transition:opacity 0.2s">@SydneyDaddy1</text>

        <!-- "keywords shared" label on the center zone -->
        <text x={REGION_LABELS.all.x} y={REGION_LABELS.all.y + 16}
          text-anchor="middle"
          font-family="Montserrat,sans-serif" font-size="8.5" letter-spacing="0.08em"
          fill="rgba(255,255,255,0.3)"
          style="pointer-events:none">keywords</text>
      </svg>

      <!-- HTML tooltip (position: fixed aligns with clientX/Y) -->
      {#if hoverRegion && tooltipKws.length}
        <div class="tooltip" style="left:{clientX+16}px; top:{clientY - 12}px"
          transition:fade={{duration:100}}>
          <div class="tt-head">{tooltipTitle}</div>
          <div class="tt-list">
            {#each tooltipKws as kw}
              <div class="tt-row">
                <span class="tt-cn">{kw.cn}</span>
                <span class="tt-en">{kw.en}</span>
              </div>
            {/each}
          </div>
        </div>
      {/if}
    </div>
  </div>

  <!-- ── FRAME 2: Bubble Chart ─────────────────────────────────────────────────── -->
  {:else if view==='bubble' && selCirc}
  <div transition:fade={{duration:350}}>
    <button class="back-btn" on:click={()=>{view='venn';selectedId=null;}}>
      ← Back to Overview
    </button>

    <div class="kv-head" style="padding-top:2.5rem">
      <h2 class="kv-title" style="color:{selCirc.color}">{selCirc.handle}</h2>
      <p class="kv-sub">
        {selCirc.posts.toLocaleString()} total posts · bubble size = % of tweets containing keyword
      </p>
    </div>

    <div class="bubble-wrap">
      <svg viewBox="0 0 720 460" class="bubble-svg">
        <defs>
          <filter id="hd2" x="-8%" y="-8%" width="116%" height="116%">
            <feTurbulence type="fractalNoise" baseFrequency="0.05" numOctaves="2" result="n"/>
            <feDisplacementMap in="SourceGraphic" in2="n" scale="3"
              xChannelSelector="R" yChannelSelector="G"/>
          </filter>
        </defs>

        <!-- Global background scatter dots -->
        {#each bubbles as b}
          {#if b.r > 30}
            <circle cx={b.x + (Math.random()-0.5)*b.r*0.6}
                    cy={b.y + (Math.random()-0.5)*b.r*0.6}
                    r="1" fill={selCirc.color} opacity="0.08"/>
          {/if}
        {/each}

        <!-- Keyword bubbles -->
        {#each bubbles as b}
          <!-- Dashed organic circle -->
          <circle cx={b.x} cy={b.y} r={b.r}
            fill={selCirc.dim}
            stroke={selCirc.color}
            stroke-width={b.r > 36 ? 1.8 : 1.2}
            stroke-dasharray={b.r > 36 ? '7 3.5' : '4.5 2.5'}
            filter="url(#hd2)"
            opacity="0.88"/>

          <!-- Chinese text (inside if large, above if small) -->
          {#if b.r >= 30}
            <text
              x={b.x} y={b.y + (b.r > 42 ? -4 : 3)}
              text-anchor="middle" dominant-baseline="middle"
              font-family="Montserrat,sans-serif"
              font-size={b.r > 44 ? 13 : b.r > 34 ? 11 : 9}
              font-weight="700"
              fill="rgba(255,255,255,0.92)"
              style="pointer-events:none">
              {b.cn}
            </text>
            {#if b.r > 42}
              <text
                x={b.x} y={b.y + 12}
                text-anchor="middle"
                font-family="Montserrat,sans-serif"
                font-size="8" font-weight="400"
                fill="rgba(255,255,255,0.42)"
                style="pointer-events:none">
                {b.en}
              </text>
            {/if}
          {:else}
            <text
              x={b.x} y={b.y - b.r - 5}
              text-anchor="middle"
              font-family="Montserrat,sans-serif"
              font-size="8.5" font-weight="700"
              fill="rgba(255,255,255,0.75)"
              style="pointer-events:none">
              {b.cn}
            </text>
          {/if}
        {/each}
      </svg>

      <!-- Size legend -->
      <div class="bubble-legend">
        <span class="legend-dot" style="--s:8px; background:{selCirc.color}22; border-color:{selCirc.color}66"></span>
        <span class="legend-dot" style="--s:16px; background:{selCirc.color}22; border-color:{selCirc.color}66"></span>
        <span class="legend-dot" style="--s:24px; background:{selCirc.color}22; border-color:{selCirc.color}66"></span>
        <span class="legend-label">→ higher % of tweets</span>
      </div>
    </div>
  </div>
  {/if}

</section>

<style>
  .kv {
    background: #03050c;
    padding: 6vh 4vw 8vh;
    font-family: 'Montserrat', sans-serif;
    position: relative;
    min-height: 80vh;
  }

  /* ── Header ── */
  .kv-head {
    text-align: center;
    margin-bottom: 3vh;
  }

  .kv-title {
    font-size: clamp(1.2rem, 3vh, 2rem);
    font-weight: 800;
    color: #fff;
    margin: 0;
  }

  .kv-sub {
    font-size: 0.78rem;
    color: rgba(255,255,255,0.38);
    margin-top: 0.5rem;
  }

  /* ── Account legend row ── */
  .legend-row {
    display: flex;
    justify-content: center;
    gap: 12px;
    flex-wrap: wrap;
    margin-bottom: 3.5vh;
  }

  .acct-pill {
    font-family: 'Montserrat', sans-serif;
    font-size: 0.7rem;
    font-weight: 700;
    letter-spacing: 0.05em;
    padding: 5px 14px;
    border-radius: 999px;
    border: 1px solid;
    cursor: pointer;
    transition: opacity 0.15s, transform 0.15s;
  }

  .acct-pill:hover {
    opacity: 0.75;
    transform: translateY(-1px);
  }

  /* ── Venn SVG ── */
  .venn-wrap {
    position: relative;
    max-width: 820px;
    margin: 0 auto;
  }

  .venn-svg {
    width: 100%;
    height: auto;
    cursor: crosshair;
    display: block;
  }

  /* ── Tooltip ── */
  .tooltip {
    position: fixed;
    z-index: 999;
    background: rgba(8, 10, 24, 0.96);
    border: 1px solid rgba(255,255,255,0.12);
    border-radius: 12px;
    padding: 11px 15px;
    min-width: 170px;
    max-width: 240px;
    box-shadow: 0 6px 32px rgba(0,0,0,0.55);
    pointer-events: none;
  }

  .tt-head {
    font-size: 0.65rem;
    font-weight: 700;
    text-transform: uppercase;
    letter-spacing: 0.12em;
    color: rgba(255,255,255,0.45);
    margin-bottom: 9px;
    padding-bottom: 7px;
    border-bottom: 1px solid rgba(255,255,255,0.08);
  }

  .tt-list { display: flex; flex-direction: column; gap: 6px; }

  .tt-row { display: flex; align-items: baseline; gap: 7px; }

  .tt-cn {
    font-size: 0.88rem;
    font-weight: 600;
    color: rgba(255,255,255,0.92);
  }

  .tt-en {
    font-size: 0.66rem;
    color: rgba(255,255,255,0.38);
    white-space: nowrap;
  }

  /* ── Back button ── */
  .back-btn {
    position: absolute;
    top: 0;
    left: 0;
    background: rgba(255,255,255,0.05);
    border: 1px solid rgba(255,255,255,0.12);
    color: rgba(255,255,255,0.65);
    font-family: 'Montserrat', sans-serif;
    font-size: 0.76rem;
    font-weight: 600;
    letter-spacing: 0.04em;
    padding: 8px 16px;
    border-radius: 8px;
    cursor: pointer;
    transition: background 0.18s, color 0.18s;
  }

  .back-btn:hover {
    background: rgba(255,255,255,0.09);
    color: #fff;
  }

  /* ── Bubble chart ── */
  .bubble-wrap {
    max-width: 780px;
    margin: 0 auto;
    position: relative;
  }

  .bubble-svg {
    width: 100%;
    height: auto;
    display: block;
  }

  /* ── Bubble legend ── */
  .bubble-legend {
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 6px;
    margin-top: 1rem;
  }

  .legend-dot {
    display: inline-block;
    width: var(--s);
    height: var(--s);
    border-radius: 50%;
    border: 1px dashed;
    flex-shrink: 0;
  }

  .legend-label {
    font-size: 0.68rem;
    color: rgba(255,255,255,0.3);
    letter-spacing: 0.05em;
    margin-left: 4px;
  }
</style>
